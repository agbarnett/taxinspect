###  to do
#- increase in effort based on extra time per paper
#- once get good level of auditing remove impact on: i) parents, ii) children and observe difference
...
taxinspect = function(
  ...
  select = eligible[rand, ] # randomly selected lab for auditing
...
#  frame.without = frame() # remove lab in question
  ...
  audit.error = 0 # option to add measurement error here to select$cum.FP
  ...
  if(kill == F){
    ...
    select$e_i = select$e_i + increase_e # increase effort in audited lab (assume audit triggers better future behavior)
